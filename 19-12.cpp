...
//the best way to make change for a sepcified amount 
#include <iostream>
using namespace std;

//constants
const int MAX_COINS_CHANGE = 100;    //max number of coins to give in char 
const int MAX_COIN_VALUES = 6;       //max number of coin values
const int NO_SOLUTION = INT_MAX;    //Indicates no solution 

//function prototypes 
void makeChanges(int, int, int[], int);

//coin values = global array of coin values to choose from 
...
//**************************************************************************
//Funciton makeChange. This function uses the following parameters: 
...
void makeChange(int coinsLeft, int amount, int coinsUsed[], int numCoinsUsed)
{
	...
	if (coinsLeft == 0)   //If no more coins are left
		return;
	else if (amount < 0)   //If amount to make change for its negative
		return;
	else if (amount == 0) //if the solution is found 
	{
		//stor bestCoins if best
